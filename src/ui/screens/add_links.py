...
class AddLinksScreen(BaseScreen):
    ...
    def screen_content(self):
        ...
        yield Input(placeholder="Paste links separated by space or comma...", id="link_input")
        yield ListView(id="links_list")
        yield Button("Download", id="download")

    def on_mount(self):

        self.downloader = Downloader(self.settings, self.app.progress_manager)
    ...
    def on_input_submitted(self, event: Input.Submitted):
        ...
        self.app.notify(f"{len(links)} link(s) added")
    ...
    def update_links_list(self):
        ...
        for index, link in enumerate(self.link_manager.get_all()):
            ...
            links_list.append(ListItem(Label(f"{index + 1}. {link}")))
    ...
    def on_button_pressed(self, event: Button.Pressed):
        ...
        Thread(target=self.downloader.download,args=(links.copy(),),daemon=True).start()

        self.app.notify("Download started")
    ...
    def action_remove_link(self):
        ...
        if links_list.index is None:
            ...
            self.app.notify("No link selected")
